...
def Separable_conv2d(inputs, filters, kernel_size=3, strides=1, padding='VALID', Use_bias=tf.zeros_initializer()):
    ...
    return tf.contrib.layers.separable_conv2d(inputs,
                                              num_outputs=filters,
                                              kernel_size=kernel_size,
                                              depth_multiplier=1,
                                              stride=strides,
...
def dwise_conv(input, k_h=3, k_w=3, channel_multiplier=1,
               strides=[1, 1, 1, 1],
               padding='VALID',
               name='dwise_conv',
               bias=True):
    ...
    with tf.variable_scope(name):
        ...
        conv = tf.nn.depthwise_conv2d(input, w, strides, padding,
                                      rate=None,
                                      name=name,
                                      data_format=None)
        if bias:

...
def parse_args():
    ...
    parser.add_argument('--checkpoint_dir',
                        type=str,
                        default='../checkpoint/' + 'AnimeGANv2_Hayao_lsgan_300_300_1_2_10_1_lite',
...
def main(checkpoint_dir, style_name):
    ...
    with tf.variable_scope("generator", reuse=False):
        if 'lite' in checkpoint_dir:
            _ = generator_lite(placeholder).fake
        else:
            _ = generator(placeholder).fake

    generator_var = [var for var in tf.trainable_variables()

    ...
    with tf.variable_scope(scope):
        ...
        if sn:
            w = tf.get_variable("kernel",
                                shape=[kernel, kernel, x.get_shape()[-1],
                                       channels],
...
def discriminator_loss(loss_func, real, gray, fake, real_blur, dataset):
    real_loss = 0
    gray_loss = 0
    fake_loss = 0
    real_blur_loss = 0
    ...
    if loss_func == 'wgan-gp' or loss_func == 'wgan-lp':
        ...
        gray_loss = tf.reduce_mean(gray)  # XXX Treat grayscale anime as fake?
    ...
    if loss_func == 'gan' or loss_func == 'dragan':
        real_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(real),
                                                                           logits=real))
        gray_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(gray),
                                                                           logits=gray))
        fake_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(fake),
                                                                           logits=fake))
        real_blur_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(abels=tf.zeros_like(real_blur),
                                                                                logits=real_blur))
    ...
    if dataset.lower() == "hayao":
        w_real = 1.2
        w_fake = 1.2
        w_gray = 1.2
        w_real_blur = 0.8
    elif dataset.lower() == "paprika":
        w_real = 1.0
        w_fake = 1.0
        w_gray = 1.0
        w_real_blur = 0.005
    elif dataset.lower() == "shinkai":
        w_real = 1.7
        w_fake = 1.7
        w_gray = 1.7
        w_real_blur = 1.0

    loss = w_real * real_loss \
        + w_fake * fake_loss \
        + w_gray * gray_loss \
        + w_real_blur * real_blur_loss
    return loss
...
def generator_loss(loss_func, fake):
    ...
    if loss_func == 'gan' or loss_func == 'dragan':
        fake_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(fake),
                                                                           logits=fake))
...
def color_loss(con, fake):
    ...
    return L1_loss(con[:, :, :, 0], fake[:, :, :, 0]) \
        + Huber_loss(con[:, :, :, 1], fake[:, :, :, 1]) \
        + Huber_loss(con[:, :, :, 2], fake[:, :, :, 2])

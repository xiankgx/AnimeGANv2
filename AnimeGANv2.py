...
from net import D_net, generator, generator_lite
...
class AnimeGANv2(object):
    ...
    def gradient_penalty(self, real, fake, scope="discriminator"):
        ...
        elif self.gan_type.__contains__('gp') \
                or self.gan_type == 'dragan':
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.epoch):
            for idx in range(int(self.dataset_num / self.batch_size)):
                # Get data
                anime, anime_smooth, real = self.sess.run([anime_img_op,
                                                           anime_smooth_op,
                                                           real_img_op])

                # Bundle data for training
                train_feed_dict = {
                    self.real: real[0],
                    self.anime: anime[0],
                ...
                if epoch < self.init_epoch:
                    real_images, generator_images, _, v_loss, summary_str \
                        = self.sess.run([self.real,
                                         self.generated,
                                         self.init_optim,
                                         self.init_loss,
                                         self.V_loss_merge],
                ...
                # GAN training phase
                    ...
                    # Update discriminator once every 'training_rate' steps
                    ...
                    # Update generator
                    real_images, generator_images, _, g_loss, summary_str \
                        = self.sess.run([self.real,
                                         self.generated,
                                         self.G_optim,
                                         self.Generator_loss,
